...
class MultiApp
{
        ...
        if ($appName || ($scriptName && !in_array($scriptName, ['index', 'router', 'think']))) {
            ...
            if (!$this->app->http->isBind()) {
                ...
                if (isset($map[$name])) {
                    ...
                    $extraPath = $this->app->getRootPath() . $this->app->config->get('app.extra_app', 'system') . '/' .  $appName . DIRECTORY_SEPARATOR;

                    if (!is_dir($appPath) && !is_dir($extraPath)) {
    ...
    protected function setApp(string $appName): void {
        ...
        } else if (is_dir($extraPath)) {
            $this->app->setAppPath($extraPath);
            // 设置应用命名空间
            $this->app->setNamespace($this->app->config->get('app.extra_app', 'system') .'\\' . $appName);
            $this->app->setRuntimePath($this->app->getRuntimePath() . $appName . DIRECTORY_SEPARATOR);
            $this->app->http->setRoutePath($this->getRoutePath());
            //加载应用
            $this->loadApp($appName, $extraPath);
        }
    }
